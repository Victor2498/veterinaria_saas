...
async def init_db():
    ...
    async with AsyncSessionLocal() as session:
        ...
        for table, col, col_type in alterations:
            try:
                check_sql = "SELECT column_name FROM information_schema.columns WHERE table_name=:table AND column_name=:col;"
                res = await session.execute(text(check_sql), {"table": table, "col": col})
                if not res.scalar():
                    print(f"Adding column {col} to {table}...")
                    # ALTER TABLE doesn't support parameters for column names/types.
                    # As this is an internal admin migration script with hardcoded arrays above, it's safe.
        ...
        for idx_name, table, columns in indexes:
            try:
                ...
                check_idx = "SELECT indexname FROM pg_indexes WHERE tablename = :table AND indexname = :idx_name;"
                res = await session.execute(text(check_idx), {"table": table, "idx_name": idx_name})
                if not res.scalar():
                    print(f"Creating index {idx_name} on {table}...")
                    # DDL does not support bound parameters, but the data is hardcoded above.
